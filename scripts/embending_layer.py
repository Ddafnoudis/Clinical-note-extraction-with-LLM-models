...
def embedding_layer(model: Dict, vocab_size: int, dim: int, tokens: torch.Tensor, norm_eps: int)-> torch.Tensor: 
    ...
    # Copy the pre-trained token embeddings to the embedding layer
    embedding_layer.weight.data.copy_(model["tok_embeddings.weight"])
    # Convert token embedding to torch.bfloat16
    token_embeddings_unnormal = embedding_layer(tokens).to(torch.bfloat16)
    # Print shape and type of unnormalized embeddings
    print(f"\nThe unnnormalized embedding token shape is: {token_embeddings_unnormal.shape}")
    print(f"The type of unnnormalized embedding token is {type(token_embeddings_unnormal)}\n")
    ...
    print(f"Normalized token shape: {token_embeddings.shape}")
